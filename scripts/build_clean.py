...
Same division of labour as build_translation.py / build_digest.py, for the same reason: the
...
contain the source's numbers and names. Route D cannot check any of this (a digest is a third of
its input, by design) and the translate seam cannot check names (the prompt PERMITS Russifying
...
def render_md(doc: dict) -> str:
    ...
    Everything here is DERIVED, exactly like build_digest.render_md: nothing in this file was
...
def _load_json(path: Path):
    ...
    uses (runreport._load_json, build_digest._load_json)."""
    try:
