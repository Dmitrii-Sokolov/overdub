export const meta = {
  ...
  description: 'Route B / Step 2: fan out one Sonnet translator per video, deterministically',
  whenToUse: 'The overdub-sonnet-batch skill Step 2. Needs args {ids: [...], root: "D:\\\\code\\\\overdub"}.',
  phases: [
    { title: 'Translate', detail: 'one Sonnet sub-agent per video, all spawned at once' },
  ],
}

// WHY THIS EXISTS — measured on route B.
//
...
//   window — the run above died at 89% with Step 4 never executed. A workflow's agent results
//   return to THIS SCRIPT, not to the model's context.
//
//   TIME. The orchestrator generates prompts at ~8.5 s per 1000 chars (measured 2026-07-21), and
//   a wave costs `spawn total + the last agent's own window`. 403k chars of translator prompt
//   is ~57 minutes of pure generation per batch, during which agent-side speedups are worth zero.
//
// The prompts live HERE rather than in the skill: anything the orchestrator has to TYPE is the
// cost this file exists to remove, and two copies of one prompt drift. The contract is not pasted
// at all — the sub-agent reads it off disk (9.1k chars saved per spawn) under a MANDATORY-READ
// rule with an explicit stop marker, so an agent that could not read it aborts loudly instead of
// translating to its own taste.

// Accept `args` as either an object or a JSON string — the caller stringified it in all 8 attempts
// on the first real invocation of this pattern (2026-07-21), and the failure mode is an empty
// fan-out that reports success. A failed parse becomes null on purpose: that falls through to the
// loud throw.
let ARGS = args
...
if (!IDS.length) {
  throw new Error(
    'translate-batch: args.ids is empty. Pass the RESUME-FILTERED list ' +
    'from Step 2, e.g. {ids: ["abc12345678"], root: "D:\\\\code\\\\overdub"}. ' +
...
log(`fanning out ${IDS.length} translators (all at once)`)
...
const results = await parallel(IDS.map((id) => () =>
  ...
  // 2.4k chars and a worst case of 13.5k, so the instruction alone is not a mechanism.
    ...
    return { id, status: parseStatus(full), text: full.slice(0, 200).trim() }
  })
))
...
// worth less than the file it left behind (2026-07-20).
const done = []
const failed = []
const incomplete = []
const unclear = []
...
IDS.forEach((id, i) => {
  const r = results[i]
  if (!r) failed.push(id)                                      // runtime dropped the agent
  else if (r.status === 'contract') failed.push(id)
  else if (r.status === 'incomplete') incomplete.push(id)
  else if (r.status === 'ok') done.push(id)
  else unclear.push(id)
})

log(`translated ${done.length}/${IDS.length}` +
