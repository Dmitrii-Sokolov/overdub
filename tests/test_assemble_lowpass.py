...
track loses its vocoder hiss while a 24 kHz track is left untouched (the filter would ride
that band edge and recolour the voice for nothing). run() writes the stamp from this
function and done() re-derives it from the same one — a legacy stamp with no lowpass_hz
field reads as None and must NOT churn a 24 kHz run, whose effective cutoff is also None
...
def test_legacy_low_rate_stamp_does_not_churn() -> None:
    # The done() comparison, spelled out: absent field vs freshly derived cutoff.
    legacy_stamp: dict = {"synth_key": "k", "units_key": "u"}
    assert legacy_stamp.get("lowpass_hz") == effective_lowpass(Config().dub_lowpass_hz, _LOW_SR)
