"""Unit tests for `--transcribe-only` = download (AUDIO ONLY) → transcribe → stop.

Run: .venv-asr/Scripts/python.exe -X utf8 tests/test_transcribe_only.py   (or via pytest)
...
The failure classes guarded here, in the order they would actually bite:
  ...
  SILENT FULL DOWNLOAD — `-f bestaudio` must carry no '/best' tail. The fallback pulls a
    progressive VIDEO stream at ~20x the bytes, i.e. the mode doing the exact thing it exists
    to prevent.
  DESYNCHRONIZED MODE — a truncated stage list with a FULL download (100 GB for a transcript
    pass) or a full list with an audio-only download (mux fails eight hours in).
    transcribe_only_stages constructs both facts in one expression; the stage-list tests pin
    that they stay one fact.
  TORN source.wav — both done() gates are bare existence checks, so an interrupted extraction
    leaves a truncated wav that every later resume accepts as complete. This mode's gate is the
    first in the pipeline with no sibling artifact to contradict it.
  A NO-OP FLAG — --transcribe-only beside --only or --repair-asr would silently do nothing at 2am.
  A LOST TRANSCRIPT ON PROMOTION — the re-download rewrites source.wav; nothing downstream may
    notice, and sentences.json must survive untouched or the transcript pass paid for nothing.
"""
...
from overdub.stages import all_stages, transcribe_only_stages  # noqa: E402
...
def _usage_error(fn, *a, **kw) -> str:
    ...
    SystemExit" is true whether or not the guard exists. Drop (args.transcribe_only,
    "--transcribe-only") from the repair-exclusion tuple and control reaches
    sys.exit(_run_repair(...)) — which, on a workdir with no sentences.json and ids=None, calls
    the missing transcript a legitimate '[skip]' and returns 0. _exits() reports True either
    way: a green test for a deleted feature, i.e. the exact silent no-op this file's docstring
    forbids. argparse exits 2 and _run_repair only ever returns 0/1/3, so the CODE separates
    them; the message is asserted too, so the test pins WHICH guard fired rather than merely
    "some usage error".
    """
    buf = io.StringIO()
    with contextlib.redirect_stdout(buf), contextlib.redirect_stderr(buf):
...
def test_transcribe_only_with_only_is_a_usage_error() -> None:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        assert _exits(cli.main, [URL, "--transcribe-only", "--only", "transcribe",
                                 "--config", str(cfgp)])
        assert not (tmp / "work").exists()


def test_transcribe_only_with_repair_asr_is_a_usage_error() -> None:
    # Repair runs no stages, so --transcribe-only beside it is a flag that silently does nothing.
    # Both orderings, because argparse order must not decide whether a no-op is caught.
    with tempfile.TemporaryDirectory() as d:
        tmp = Path(d)
        cfgp = _cfg_file(tmp)
        for argv in ([URL, "--transcribe-only", "--repair-asr", "auto"],
                     [URL, "--repair-asr", "auto", "--transcribe-only"]):
            err = _usage_error(cli.main, argv + ["--config", str(cfgp)])
            assert "--transcribe-only does not apply to --repair-asr" in err
...
def test_transcribe_only_with_force_is_legal() -> None:
    # --force with --transcribe-only re-runs BOTH stages (run_pipeline bypasses done() for the
    # whole list), not just the fetch. Legal, and the help text says so.
    rec = _Recorder()
    ...
    try:
        with tempfile.TemporaryDirectory() as d:
            ...
            assert _exits(cli.main, ["--batch", str(q), "--transcribe-only", "--force",
                                     "--config", str(_cfg_file(tmp))])
    finally:
        cli._run_batch_stage_major = real
    assert len(rec.calls) == 1
    assert rec.calls[0][1]["transcribe_only"] is True and rec.calls[0][1]["force"] is True


def test_transcribe_only_with_video_major_and_no_batch_is_still_a_usage_error() -> None:
    # --transcribe-only must not sneak a single-video run past the --video-major guard.
    with tempfile.TemporaryDirectory() as d:
        cfgp = _cfg_file(Path(d))
        assert _exits(cli.main, [URL, "--transcribe-only", "--video-major",
                                 "--config", str(cfgp)])


def test_transcribe_only_single_url_reaches_the_driver() -> None:
    seen: list = []
    real = cli._run_one
    cli._run_one = lambda url, cfg, **kw: seen.append((url, kw)) or None
    try:
        with tempfile.TemporaryDirectory() as d:
            _quiet(cli.main, [URL, "--transcribe-only", "--config", str(_cfg_file(Path(d)))])
    finally:
        cli._run_one = real
    assert len(seen) == 1
    assert seen[0][0] == URL and seen[0][1]["transcribe_only"] is True


# --- stage list ---------------------------------------------------------------------

def test_stage_list_is_download_then_transcribe() -> None:
    assert [s.name for s in transcribe_only_stages(Config())] == ["download", "transcribe"]


def test_stage_list_is_a_strict_prefix_of_all_stages() -> None:
    # A promoted video re-enters the FULL pipeline on the artifacts these two produced. If the
    # truncated list ever stops being a prefix, promotion resumes onto a workdir built by a
    # different sequence and the fast-skips stop meaning what they say.
    cfg = Config()
    assert ([s.name for s in transcribe_only_stages(cfg)]
            == [s.name for s in all_stages(cfg)][:2])


def test_transcribe_only_download_is_audio_only_and_the_full_one_is_not() -> None:
    # The desynchronization guard: truncated list + full download = 100 GB for a transcript
    # pass; full list + audio-only download = mux failing at the end of an eight-hour run.
    cfg = Config()
    assert transcribe_only_stages(cfg)[0].audio_only is True
    assert all_stages(cfg)[0].audio_only is False


def test_transcribe_only_runs_no_stage_after_transcribe() -> None:
    """Through cli.main, so what is pinned is that main SELECTS the truncated list.

    Patching cli.transcribe_only_stages (not cli.all_stages) is the point: patching the latter
    would leave the mode untested and the assertion green.
    """
    trace: list = []
    short = [FakeStage(n, trace) for n in ("download", "transcribe")]
    full = short + [FakeStage(n, trace) for n in
                    ("translate", "synthesize", "verify", "assemble", "separate", "mux")]
    real_short, real_all = cli.transcribe_only_stages, cli.all_stages
    cli.transcribe_only_stages = lambda cfg: short
    cli.all_stages = lambda cfg: full
    try:
        with tempfile.TemporaryDirectory() as d:
            _quiet(cli.main, [URL, "--transcribe-only", "--config", str(_cfg_file(Path(d)))])
    finally:
        cli.transcribe_only_stages, cli.all_stages = real_short, real_all
...
def test_audio_download_fast_skips_when_source_wav_exists() -> None:
    # Resume: a second --transcribe-only pass over the same queue must cost seconds, not a
    # re-fetch.
    with tempfile.TemporaryDirectory() as d:
        ctx = _ctx(Path(d))
        ctx.work.source_audio.write_bytes(b"wav")
        ...
        def boom(_ctx):
            raise AssertionError("audio download re-ran despite source.wav being present")
        ...
        if argv[0] == "yt-dlp":
            ...
            if sidecar:
                (media.parent / f"{media.name}.info.json").write_text(
                    json.dumps({"title": "Fetched Title", "duration": 2530.0}), encoding="utf-8")
...
def test_audio_ytdlp_argv() -> None:
    calls: list = []
    with tempfile.TemporaryDirectory() as d:
        ctx = _ctx(Path(d))
...
def test_audio_ffmpeg_argv_and_atomic_write() -> None:
    calls: list = []
    with tempfile.TemporaryDirectory() as d:
        ctx = _ctx(Path(d))
...
def test_audio_info_json_is_normalized() -> None:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        assert doc["title"] == "Fetched Title" and doc["duration"] == 2530.0
        ...
        try:
            assert cli._title_of(ctx) == "Fetched Title"
        finally:
            c.subprocess.run = real


def test_audio_info_json_prefers_the_fresh_sidecar() -> None:
...
def test_audio_download_fails_loud_on_an_ambiguous_media_glob() -> None:
...
def test_thumbnail_does_not_trip_the_ambiguous_media_glob() -> None:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        _with_fake_subprocess(body)
        assert ctx.work.source_audio.exists()
        # The sidecar is CONSUMED by the stage (2026-07-22): ensure_thumb_local scales it
        # into thumb.jpg and drops the full-size original. Under the fake subprocess no real
        # scaling happens — what is pinned here is that the stage reaches for the sidecar at
        # all, not the pixels.
        assert not (ctx.work.root / "source.audio.jpg").exists()


def test_the_full_download_asks_for_a_thumbnail_too() -> None:
    # INBOX 2026-07-22: these flags lived only on the audio branch, so a fully-fetched video had
    # no preview bytes anywhere on disk and the queue page rendered it pictureless.
...
def test_audio_download_cleans_a_stale_audio_file_before_fetching() -> None:
    calls: list = []
    with tempfile.TemporaryDirectory() as d:
        ctx = _ctx(Path(d))
...
def test_promotion_does_not_retranscribe() -> None:
    # The REAL TranscribeStage.done(), not a fake: the whole point of the pre-pass is that the
    # expensive transcription is paid once. done() is a bare sentences.json existence check —
...
def _drive_main(tmp: Path, argv_extra: list[str], short, full):
    """cli.main over a 3-URL queue with both stage lists patched. Returns (code, output).

    _export_output is deliberately NOT patched. The mode's finalize is _transcript_status, so
    the real _export_output is never reached anyway — but patching it to a success value is what
    lets a mutant that reverts `finalize` to _export_output return "out.mkv" and keep every
    assertion below green. The status feature could then be deleted wholesale with a green suite.
    """
    q = tmp / "queue.txt"
    q.write_text("\n".join(URLS), encoding="utf-8")
    real_short, real_all = cli.transcribe_only_stages, cli.all_stages
    ...
    try:
        with contextlib.redirect_stdout(buf), contextlib.redirect_stderr(buf):
            try:
                cli.main(["--batch", str(q), "--transcribe-only",
                          "--config", str(_cfg_file(tmp))] + argv_extra)
            except SystemExit as e:
                code = e.code
    finally:
        cli.transcribe_only_stages, cli.all_stages = real_short, real_all
...
def test_transcribe_only_runs_in_stage_major_order() -> None:
    trace: list = []
    short = [FakeStage(n, trace) for n in ("download", "transcribe")]
    with tempfile.TemporaryDirectory() as d:
        code, _ = _drive_main(Path(d), [], short, short + [FakeStage("mux", trace)])
    assert code == 0
    assert _runs(trace) == [(s, v) for s in ("download", "transcribe") for v in VIDS]


def test_transcribe_only_runs_in_video_major_order() -> None:
    trace: list = []
    short = [FakeStage(n, trace) for n in ("download", "transcribe")]
    with tempfile.TemporaryDirectory() as d:
        code, out = _drive_main(Path(d), ["--video-major"], short,
                                short + [FakeStage("mux", trace)])
    assert code == 0
    assert _runs(trace) == [(s, v) for v in VIDS for s in ("download", "transcribe")]
    # video-major picks _transcript_status too, via _run_one — BOTH drivers must cover the
    # finalize wiring, or a revert in one order hides behind the other order's test
    assert out.count("transcribed · ") == 3


def test_clean_transcribe_only_batch_exits_zero() -> None:
    # Absence of output.mkv is NOT a failure in this mode — the whole contract is that there
    # is none. No new exit codes: 0/1/2/3 keep their meanings.
    trace: list = []
    short = [FakeStage(n, trace) for n in ("download", "transcribe")]
    with tempfile.TemporaryDirectory() as d:
        code, out = _drive_main(Path(d), [], short, short)
    assert code == 0
    assert "3 ok, 0 failed, 0 unfinished" in out
    # POSITIVE: every row came from _transcript_status. The bare `not in` below cannot pin the
    # finalize CHOICE on its own — _export_output returning any truthy name satisfies it too,
    # so without this the whole status feature could be reverted with a green suite.
    assert out.count("transcribed · ") == 3
    assert "(no output.mkv)" not in out


def test_transcribe_only_failure_exits_1_and_stop_exits_3() -> None:
    ...
    trace: list = []
    short = [FakeStage("download", trace), Failing("transcribe", trace)]
    with tempfile.TemporaryDirectory() as d:
        code, out = _drive_main(Path(d), [], short, short)
    assert code == 1 and "2 ok, 1 failed" in out

    trace = []
    short = [Stopping("download", trace), FakeStage("transcribe", trace)]
    with tempfile.TemporaryDirectory() as d:
        code, out = _drive_main(Path(d), [], short, short)
    assert code == 3 and "STOP file honored" in out


def test_status_line_reports_sentences_and_duration() -> None:
    with tempfile.TemporaryDirectory() as d:
        ctx = _ctx(Path(d))
        _sentences(ctx.work, n=431)
        ctx.work.info_json.write_text(json.dumps({"duration": 2530.0}), encoding="utf-8")
        line = cli._transcript_status(ctx)
        assert "transcribed" in line and "431 sentences" in line
        assert "42:10" in line                            # 2530 s, M:SS
        assert "no output.mkv" not in line


def test_status_reports_an_unreadable_transcript_instead_of_raising() -> None:
    # _transcript_status is the stage-major driver's `finalize`; raising there would turn a
    # perfectly transcribed video into a FAIL row.
    with tempfile.TemporaryDirectory() as d:
        ctx = _ctx(Path(d))
        ctx.work.sentences.write_text("{not json", encoding="utf-8")
        assert cli._transcript_status(ctx) == "transcribed · no readable sentences.json"


def test_status_falls_back_to_sentence_ends_for_duration() -> None:
    with tempfile.TemporaryDirectory() as d:
        ctx = _ctx(Path(d))
        _sentences(ctx.work, n=5)                         # last end = 5.0
        assert "0:05" in cli._transcript_status(ctx)


def test_finish_sweep_writes_no_run_json_and_no_output() -> None:
    # The property the queue page depends on: build_run_report returns None and
    # self-clears run.json when report.json and translation.json are both absent, which is
    # exactly a transcript-only workdir's shape.
    trace: list = []
    short = [FakeStage(n, trace) for n in ("download", "transcribe")]
    with tempfile.TemporaryDirectory() as d:
        tmp = Path(d)
        code, _ = _drive_main(tmp, [], short, short)
        assert code == 0
        for v in VIDS:
...
if __name__ == "__main__":
    for name, fn in sorted(globals().items()):
        if name.startswith("test_") and callable(fn):
            fn()
            print(f"ok  {name}")
    print("all transcribe-only tests passed")
