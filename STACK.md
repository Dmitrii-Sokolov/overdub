...
- **`WhisperModel(..., num_workers=N)` is a CONSTRUCTION-time knob** that becomes ctranslate2's `inter_threads` (1.2.1 `transcribe.py:695`), and the docstring (654-657) documents calling `transcribe()` from N Python threads to use it. The SEQUENTIAL path is the thread-safe one — it keeps `last_speech_timestamp` as a local (1152), while `BatchedInferencePipeline` keeps it on `self` (117). Exposed as a `load_whisper(..., num_workers=)` keyword for the sweep harness only; the pipeline is strictly sequential and never passes it, so it is deliberately not a Config key (DECISIONS 2026-07-22).
- **`BatchedInferencePipeline` silently overrides what you pass it** (1.2.1): it accepts `condition_on_previous_text` in its signature (277) and then hardcodes `False` into `TranscriptionOptions` (547), and it forces `max_speech_duration_s=chunk_length` (404-409), `hallucination_silence_threshold=None` and `max_initial_timestamp=0.0`. `word_timestamps` DOES survive batching (161-162, 545). Since `condition_on_previous_text=True` is what buys this pipeline its punctuation (DECISIONS 2026-07-17), batching is not a drop-in speed lever here.
